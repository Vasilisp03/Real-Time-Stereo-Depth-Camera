...
int main(int argc, char** argv) {
    ...
    hls_out.create(in_imgL.rows, in_imgL.cols, CV_16UC1);
    ...
    size_t image_out_size_bytes = in_imgL.rows * in_imgL.cols * sizeof(unsigned short);
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel kernel(program, "semiglobalbm_accel", &err));
    ...
    cv::Mat diff;
    diff.create(in_imgL.rows, in_imgL.cols, CV_16UC1);
    int cnt = 0;
