...
// clang-format off
        ...
        if (col >= 2) {
            ...
            // Required so xFSGBMcomputecost can compute an AD-Census fused
            // cost; the census computation itself is unchanged.
...
// clang-format off
        ...
        // clang-format on
        censusVal = xFComputeTransform5x5<DEPTH_SRC, DEPTH_DST>(src_buf);
        // Packed with raw center pixel — see note in xFProcessCensusTransform5x5 above.
        _dst_mat.write((XF_SNAME(WORDWIDTH_DST))(((ap_uint<32>)src_buf[2][2] << 24) | (ap_uint<32>)censusVal));
...
/**
 * xFSGBMcomputecost : AD-Census fused matching cost.
 * Computes a Hamming-distance cost on the 24-bit census descriptor AND an
 * Absolute-Difference cost on the raw pixel value, then fuses them through
 * rho_census_lut / rho_ad_lut (see xf_sgbm_adcensus_lut.h). This replaces
 * the original census-only Hamming cost to improve robustness in low-texture
 * regions while retaining census's tolerance to lighting differences.
 */
...
// clang-format off
    #pragma HLS bind_storage variable=Lr type=RAM_T2P impl=BRAM
...
// clang-format off
    ...
    xFSGBMcomputecost<NDISP, PU, ROWS, COLS>(_src_census24_l, _src_census24_r, _src_raw_l, _src_raw_r, _cost, height,
                                             width);
