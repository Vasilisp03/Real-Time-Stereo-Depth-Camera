...
template <typename T>
T min_of_4_wide(T a, T b, T c, T d) {
#pragma HLS INLINE
...
void xFSGBMoptimization(hls::stream<ap_uint<8> > _cost[PU],
                        ...
                        uint8_t p2) {
#pragma HLS INLINE OFF
#pragma HLS ARRAY_PARTITION variable=_cost complete dim=1
#pragma HLS ARRAY_PARTITION variable=_agg_cost complete dim=1

    static_assert((NDISP % PU) == 0, "NDISP must be divisible by PU");
    static const int DISP_GROUPS = NDISP / PU;

    typedef ap_uint<9> path_cost_t;
    typedef ap_uint<10> candidate_cost_t;
    typedef ap_uint<PU * 9> lr_word_t;

    static const path_cost_t MAX_PATH_COST = 511;

    // [direction][packed disparity group][column]
    lr_word_t Lr[R - 1][DISP_GROUPS][COLS];
#pragma HLS BIND_STORAGE variable=Lr type=RAM_T2P impl=URAM
#pragma HLS ARRAY_PARTITION variable=Lr complete dim=1

    path_cost_t Lr_r1[NDISP];
#pragma HLS ARRAY_PARTITION variable=Lr_r1 complete dim=1

    path_cost_t Lr_r1_tmp[PU];
#pragma HLS ARRAY_PARTITION variable=Lr_r1_tmp complete dim=1

    path_cost_t Lr_r0[NDISP];
#pragma HLS ARRAY_PARTITION variable=Lr_r0 complete dim=1

    path_cost_t tmp_store_Lr[R][PU + 2];
#pragma HLS ARRAY_PARTITION variable=tmp_store_Lr complete dim=0

    path_cost_t Lr_min[R - 1][COLS];
#pragma HLS BIND_STORAGE variable=Lr_min type=RAM_T2P impl=BRAM
#pragma HLS ARRAY_PARTITION variable=Lr_min complete dim=1

    path_cost_t r1_min = 0;

    path_cost_t tmp_Lr_min[R];
#pragma HLS ARRAY_PARTITION variable=tmp_Lr_min complete dim=1

    path_cost_t tmp_Lr_min_post[R];
#pragma HLS ARRAY_PARTITION variable=tmp_Lr_min_post complete dim=1

    path_cost_t store_lr_for_min[R][PU];
#pragma HLS ARRAY_PARTITION variable=store_lr_for_min complete dim=0

init_lr_direction:
    for (int direction = 0; direction < R - 1; ++direction) {
    init_lr_group:
        for (int group = 0; group < DISP_GROUPS; ++group) {
        init_lr_column:
            for (int col = 0; col < width; ++col) {
#pragma HLS PIPELINE II=1
                Lr[direction][group][col] = 0;
            }
        }
    }

init_horizontal_state:
    for (int disparity = 0; disparity < NDISP; ++disparity) {
#pragma HLS UNROLL
        Lr_r0[disparity] = 0;
        Lr_r1[disparity] = 0;
    }
...
init_min_direction:
    ...
    init_min_column:
        for (int col = 0; col < width; ++col) {
#pragma HLS PIPELINE II=1
            Lr_min[direction][col] = 0;
        }
    }

init_post_minimum:
    for (int direction = 0; direction < R; ++direction) {
#pragma HLS UNROLL
        tmp_Lr_min_post[direction] = 0;
    }
...
#pragma HLS PIPELINE II=1
            ...
            uint8_t min_index_unused;
            path_cost_t min_cost;

        sgbm_disparity_groups:
            for (int d = 0; d < DISP_GROUPS; ++d) {
#pragma HLS PIPELINE II=1
...
#pragma HLS LOOP_FLATTEN
                ...
                clear_previous_lane:
                    for (int direction = 0; direction < R; ++direction) {
#pragma HLS UNROLL
                        tmp_store_Lr[direction][0] = 0;
                    }

                load_horizontal_group0:
                    for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                        tmp_store_Lr[0][lane + 1] = Lr_r0[lane];
                    }

                load_buffered_group0:
                    for (int direction = 1; direction < R; ++direction) {
#pragma HLS UNROLL
                        const lr_word_t current_word =
                            Lr[direction - 1][0][co + direction - 2];

                    unpack_buffered_group0:
                        for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                            tmp_store_Lr[direction][lane + 1] =
                                (path_cost_t)current_word.range(
                                    lane * 9 + 8, lane * 9);
                        }
                    }
                ...
                load_group0_next_lane:
                    for (int direction = 1; direction < R; ++direction) {
#pragma HLS UNROLL
                        if (PU < NDISP) {
                            const lr_word_t next_word =
                                Lr[direction - 1][1][co + direction - 2];
                            tmp_store_Lr[direction][PU + 1] =
                                (path_cost_t)next_word.range(8, 0);
                        } else {
                            tmp_store_Lr[direction][PU + 1] = 0;
                        }
                    }
                ...
                load_group0_minima:
                    for (int direction = 1; direction < R; ++direction) {
#pragma HLS UNROLL
                        tmp_Lr_min[direction] =
                            Lr_min[direction - 1][co + direction - 2];
                    }

                reset_group_minima:
                    for (int direction = 0; direction < R; ++direction) {
#pragma HLS UNROLL
                        tmp_Lr_min_post[direction] = MAX_PATH_COST;
                    }
                } else {
                shift_group_boundary:
                    for (int direction = 0; direction < R; ++direction) {
#pragma HLS UNROLL
                        tmp_store_Lr[direction][0] =
                            tmp_store_Lr[direction][PU];
                        tmp_store_Lr[direction][1] =
                            tmp_store_Lr[direction][PU + 1];
                    }

                load_horizontal_group:
                    for (int lane = 1; lane < PU; ++lane) {
#pragma HLS UNROLL
                        tmp_store_Lr[0][lane + 1] =
                            Lr_r0[d * PU + lane];
                    }
...
#pragma HLS UNROLL
                        const lr_word_t current_word =
                            Lr[direction - 1][d][co + direction - 2];

                    unpack_buffered_group:
                        for (int lane = 1; lane < PU; ++lane) {
#pragma HLS UNROLL
                            tmp_store_Lr[direction][lane + 1] =
                                (path_cost_t)current_word.range(
                    ...
                    const int next_disparity = (d + 1) * PU;
                    if (next_disparity < NDISP) {
                        tmp_store_Lr[0][PU + 1] = Lr_r0[next_disparity];
                    } else {
                        tmp_store_Lr[0][PU + 1] = 0;
                    }

                load_next_group_lane:
                    for (int direction = 1; direction < R; ++direction) {
#pragma HLS UNROLL
                        if (next_disparity < NDISP) {
                            const lr_word_t next_word =
            ...
            compute_lanes:
                for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                    const ap_uint<8> cost = _cost[lane].read();
                    ap_uint<16> aggregate = 0;

                compute_directions:
                    for (int direction = 0; direction < R; ++direction) {
#pragma HLS UNROLL
                        path_cost_t lr_previous =
                            tmp_store_Lr[direction][lane];
                        const path_cost_t lr_same =
                            tmp_store_Lr[direction][lane + 1];
                        path_cost_t lr_next =
                            tmp_store_Lr[direction][lane + 2];
                        const path_cost_t previous_minimum =
                            tmp_Lr_min[direction];

                        const int disparity = d * PU + lane;

                        if (disparity == 0) {
                            lr_previous = MAX_PATH_COST - p1;
                        }
                        if (disparity >= NDISP - 1) {
                            lr_next = MAX_PATH_COST - p1;
                        }

                        const ap_uint<8> p2_value =
                            ((direction == 0) && (co == 0)) ? 0 : p2;

                        const candidate_cost_t transition_minimum =
                            min_of_4_wide<candidate_cost_t>(
                                (candidate_cost_t)lr_same,
                                (candidate_cost_t)lr_previous + p1,
                                (candidate_cost_t)lr_next + p1,
                                (candidate_cost_t)previous_minimum + p2_value);

// Signed intermediate prevents subtraction underflow.
                        ap_int<11> normalized_cost;
                        #pragma HLS BIND_OP variable=normalized_cost op=sub impl=DSP
                        normalized_cost =
                            static_cast<ap_int<11>>(transition_minimum) -
                            static_cast<ap_int<11>>(previous_minimum);

                        ap_int<11> updated_wide;
                        #pragma HLS BIND_OP variable=updated_wide op=add impl=DSP
                        updated_wide =
                            static_cast<ap_int<11>>(cost) +
                            normalized_cost;

                        path_cost_t updated_lr =
                            static_cast<path_cost_t>(updated_wide);
                        ...
                        if (direction == 0) {
                            Lr_r0[disparity] = updated_lr;
                        } else if (direction == 1) {
                            Lr_r1_tmp[lane] = updated_lr;
                        }

                        store_lr_for_min[direction][lane] = updated_lr;
                        aggregate += updated_lr;
                    }

                    _agg_cost[lane].write(aggregate);
                }

            write_packed_state:
                for (int direction = 2; direction < R; ++direction) {
#pragma HLS UNROLL
                    lr_word_t packed_word = 0;

                pack_state_lanes:
                    for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                        packed_word.range(lane * 9 + 8, lane * 9) =
                            store_lr_for_min[direction][lane];
                    }

                    Lr[direction - 1][d][co] = packed_word;
                }

                xFMinSAD<PU>::find(store_lr_for_min[0],
                                   min_index_unused,
                                   min_cost);
                if (min_cost < tmp_Lr_min_post[0]) {
                    tmp_Lr_min_post[0] = min_cost;
                }

            reduce_direction_minima:
                for (int direction = 1; direction < R; ++direction) {
#pragma HLS UNROLL
                    uint8_t direction_index_unused;
                    path_cost_t direction_minimum;
                    xFMinSAD<PU>::find(store_lr_for_min[direction],
                                       direction_index_unused,
                                       direction_minimum);
                    if (direction_minimum <
                        tmp_Lr_min_post[direction]) {
                        tmp_Lr_min_post[direction] = direction_minimum;
                    }
                }

                if (co >= 1) {
                    lr_word_t packed_r1 = 0;

                pack_previous_r1:
                    for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                        packed_r1.range(lane * 9 + 8, lane * 9) =
                            Lr_r1[d * PU + lane];
                    }
                    Lr[0][d][co - 1] = packed_r1;
                }

            update_r1:
                for (int lane = 0; lane < PU; ++lane) {
#pragma HLS UNROLL
                    Lr_r1[d * PU + lane] = Lr_r1_tmp[lane];
                }
                ...
                commit_direction_minima:
                    for (int direction = 2; direction < R; ++direction) {
#pragma HLS UNROLL
                        Lr_min[direction - 1][co] =
                            tmp_Lr_min_post[direction];
                    }
                }
            }
...
void SemiGlobalBM(xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_IN_L>& _src_mat_l,
    ...
    assert((DST_T == XF_16UC1) && " WORDWIDTH_DST must be XF_16UC1 ");
    assert((NPC == XF_NPPC1) && " NPC must be XF_NPPC1 ");
